...
import aqua.blatt1.client.SecureEndpointAsymmetric;
...
public class Broker {
	private SecureEndpointAsymmetric endpoint;
	...
	public Broker() {	
		this.endpoint = new SecureEndpointAsymmetric(Properties.PORT);
			...
			public void run() {
				for (int i = 0; i < Broker.this.collection.size(); i++) {
					...
					if (Calendar.getInstance().after(leaseEndTimestamp)) {
						executor.execute(new BrokerTask(new Message(
								new DeregisterRequest(Broker.this.collection.getClientId(i)), Broker.this.collection.getClient(i))
						));
					}
				}
			}
	...
	public class BrokerTask implements Runnable {
		...
		private synchronized void register(InetSocketAddress addr) {
			if (Broker.this.collection.containsClient(addr)) {
				...
				Broker.this.endpoint.send(addr, new RegisterResponse(Broker.this.collection.getClientId(clientIndex), LEASE_TIME_MILLIS));
				return;
			} 
